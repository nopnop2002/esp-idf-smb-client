...
void app_main(void)
{
	...
	while ((ent = smb2_readdir(smb2, dir))) {
		...
		//printf("%s %-20s %-9s %llu", asctime(localtime(&t)), ent->name, type, ent->st.smb2_size);
		//printf("%s %-20s %-9s %llu\n", timestamp, ent->name, type, ent->st.smb2_size);
		//printf("%s %-9s %-20s %llu\n", timestamp, type, ent->name, ent->st.smb2_size);
		printf("%s %-9s %llu %-20s\n", timestamp, type, ent->st.smb2_size, ent->name);
